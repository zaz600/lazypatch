...
##########################################################################
#                                                                        #
#  Utility for autoinstall FTAS patches. Version 0.8b                    #
#                                                                        #
		...
		try:
			...
			if not options.nokill:
				...
				if os.name == "nt":
					...
					out = self.ex.run("ps -ef | grep -E \"FND|POXCON|RCVOLTM|INCTM|frmweb|java\" | grep -vE \"grep|sufd\" | awk '{print $2}' | xargs kill -9", realtime_logging=False)
				elif os.name == "posix":
			...
			if not options.norestartdb:
				...
				out = self.ex.run("sqlplus -S sys/%s@%s as sysdba" % (syspwd, sidname), "shutdown immediate;\nstartup;\n\nexit;\n", False)
